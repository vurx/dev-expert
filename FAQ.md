...
**接入位置（v1.14.0 起，v1.15.0 收缩，v1.17.0 扩展）**：8 个专项已接入主流程 Step 2-6——Step 2「专项纪律预判」按领域路由预判挂接并写入执行计划；Step 3 执行触发根因闭环、依赖引入门禁与 AI 编码治理边界；Step 4 验证强制根因闭环证据与回归基线确认；Step 5 交付过 PRR/渐进式发布；Step 6 复盘强制事故复盘与阶段导航。命中即按对应步骤门禁执行，不再只是"按需加载的资料"。
